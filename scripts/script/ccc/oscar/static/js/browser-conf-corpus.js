var browser_conf = {
  ...
  "categories":{
    ...
    "document": {
          "rule": "br\/.*",
          "query": [`
            SELECT DISTINCT ?my_iri ?id_lit ?short_iri ?title ?subtitle ?year ?type ?short_type ?author ?author_browser_iri (count(?next) as ?tot) (COUNT(distinct ?cites) AS ?out_cits) (COUNT(distinct ?cited_by) AS ?in_cits) ?journal ?journal_data
            WHERE{

              		BIND(<https://w3id.org/oc/ccc/[[VAR]]> as ?my_iri) .
              		BIND('[[VAR]]' as ?short_iri) .
                  ?my_iri rdf:type ?type ; dcterms:title ?title .
                  ...
                  OPTIONAL {
                    ...
                    OPTIONAL {
                      ?my_iri frbr:partOf+ ?journal_iri .
            ...
            } GROUP BY ?my_iri ?id_lit ?short_iri ?title ?subtitle ?year ?type ?short_type ?author ?author_browser_iri ?journal ?journal_data ORDER BY DESC(?tot)`
          ],
          "links": {
          ...
          "group_by": {"keys":["title"], "concats":["author","short_type"]},
          ...
          "contents": {
            ...
            "header": [
                ...
                {"fields": ["FREE-TEXT","id_lit"], "values":["DOI: ", null], "classes":["identifiers"] },
                {"classes":["20px"]},
